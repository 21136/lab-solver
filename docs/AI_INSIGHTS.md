...
5. 未来可以从本文件的洞察人工 promote 为技能，或进一步做自动 promote
6. V3-4：`record_skill_candidates_from_run` 在 Agent run 结束时写入 `%APPDATA%/lab-solver/skill_candidates.json`（同一 error_category 或 notes_hash 7 天内 ≥2 次 → `status: pending`）
