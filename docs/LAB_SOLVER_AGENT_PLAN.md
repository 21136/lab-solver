...
overview: 模块化 Agent + 分节工作台 + 多文档/PDF；V2 **标准/深度/ReAct** 三档（2026-06-05 移除快速解题按钮，合并到 run_mode）；执行失败可增量 replan；preflight 零 LLM 预检；brief 仅点按 LLM 分类。已合并 DeepSeek 两轮评审（见附录 B/D）。用户自选 API Key。**实现进度**：V1 27/27 completed + V2 L0-L3 完成；详见 IMPLEMENTATION_PHASES.md。**架构演进**：V3 编排收敛与反馈闭环见 docs/AGENT_ARCHITECTURE_V3.md。
todos:
  - id: phase1-modules
...
- **核心原则**：不把「上传 → 解题 → 运行 → 截图 → 填表」写死；先把能力拆成**子模块**，由 AI 根据**报告全文**生成**步骤计划**，用户在桌面端**确认/勾选**后再执行。
- **画像原则**：用户画像用于**补全不确定项**与**默认参数**（语言、截图风格、是否常要 UML），**不能**在报告无依据时单独新增步骤；报告原文仍是步骤是否存在的最高优先级。
...
| `run_code`              | 编译运行并返回 stdout/stderr                | `execute_code`, `/api/run-code`                         |
| `fix_code`              | 将运行错误喂回 LLM 修代码（独立 prompt）           | 新建，复用 `llm_client`                                      |
| `screenshot_ide`        | IDE 风格截图                             | `render_ide_screenshot_file`                            |
| `screenshot_terminal`   | 纯终端图                                 | `render_terminal_image`                                 |
| `render_uml`            | PlantUML 渲染                          | `render_diagrams`                                       |
| `fill_report`           | 写回 docx                              | `do_fill`                                               |
...
| **ReAct** | `react` | ReAct Loop + **自动收尾**（UML/截图/填表）；`finalize_report` 一键工具 | 5～12（含补跑） |
...
| `screenshot` | `screenshot_ide` | IDE 风格截图 |
| `fill_report` | `fill_report` | 写入 Word（文字 + 截图 + UML） |
| **`finalize_report`** | 组合 | **一键** render_uml → screenshot（按计划）→ fill_report |
| `done` | — | 结束循环 |
...
4. `screenshot` → `fill_report`，或直接 **`finalize_report`**
5. `done`

**收尾保障**（解决「12 轮全耗在 fix_code、无图无填表」）：
...
- 主循环结束后，若 `solve_lab` 已成功且计划勾选，**自动补跑**缺失的 `render_uml` / `screenshot_*` / `fill_report`（写入 `thought_trace`，标记 `finalize: true`）。
- `run_code`/`fix_code` 累计失败 **4 次** → 注入系统提示：停止修代码，调用 `finalize_report`。
- 表格模版（实验内容格）：UML/截图经 `_insert_images_in_cell` 写入**单元格内**（非表格外 `addnext`）。见 BF21/BF22 · `V2_DOC_TEMPLATE_ADAPTATION.md`。
...
| `images_ready`       | 计划含截图步时 `images_b64` 非空                                                                                                                           | warn                        |
...
- `deep_pipeline`（`run_mode=deep`：preflight + reflect 锚定 `assignment_raw` + `max_rounds`）。
...
来源：[lab-solver/docs/deepseek的建议.md](C:/Users/21136/lab-solver/docs/deepseek的建议.md)
