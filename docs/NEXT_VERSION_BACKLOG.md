...
**关联**：[IMPLEMENTATION_PHASES.md](./IMPLEMENTATION_PHASES.md) §四 · [LAB_SOLVER_AGENT_PLAN.md](./LAB_SOLVER_AGENT_PLAN.md) · [AGENT_ARCHITECTURE_V3.md](./AGENT_ARCHITECTURE_V3.md) · [V4_MULTI_PHASE_SOLVE.md](./V4_MULTI_PHASE_SOLVE.md) · [V2_DOC_TEMPLATE_ADAPTATION.md](./V2_DOC_TEMPLATE_ADAPTATION.md) · [V2_IMAGE_INPUT.md](./V2_IMAGE_INPUT.md)
...
**技术子方案**：[V4_MULTI_PHASE_SOLVE.md](./V4_MULTI_PHASE_SOLVE.md)（分阶段 LLM + 验证沙箱，产品边界以 V5 为准）
...
| V4-1 | Agent 标准/深度统一 + SSE 子阶段 | 3～5 天 |
| V4-2 | 图表阶段 + ReAct 收敛 | 3 天 |
| V4-3 | 极速/稳妥档位 + 废弃 v1 单轮 `lab_report` | 2 天 |

**验收**：金样本 10 题首次 `run` 通过率 ≥80%；`expected_output` 来自真实 stdout。
...
**完整方案**：见 **[V2_DOC_TEMPLATE_ADAPTATION.md](./V2_DOC_TEMPLATE_ADAPTATION.md)**（DA1–DA4）· **[V2_IMAGE_INPUT.md](./V2_IMAGE_INPUT.md)**（图片/多图识题 IM1–IM5）

| 阶段 | 内容 |
|------|------|
| DA1–DA4 | 表格实训、节号映射、填表、UI |
| IM1–IM5 | 多图枚举、OCR、扫描 PDF、题图上传、Vision |
...
> **详细设计**：见 **[AGENT_ARCHITECTURE_V3.md](./AGENT_ARCHITECTURE_V3.md) §7**（与 Orchestrator / skill 候选队列一并规划，阶段 **V3-4 ✅**）。

| 项目 | 说明 |
|------|------|
| **背景** | V1 仅 `user_profile` v1：`default_language` / `screenshot_style` / `prefer_uml` + 当次 metadata |
...
**参考**：`LAB_SOLVER_AGENT_PLAN.md` §3 画像、`apply_feedback` 描述。

---

### 2. C3 — 扫描版 PDF OCR（→ 见 IM 多图流水线）

| 项目 | 说明 |
|------|------|
| **背景** | V1 `extract_pdf.py` 检测几乎无文字层 → warn，提示换 Word |
| **目标** | 扫描 PDF **可选** OCR；**多页**按页渲染/OCR 后合并（见 **[V2_IMAGE_INPUT.md](./V2_IMAGE_INPUT.md)** IM3） |
| **完整图片题** | docx 内嵌**多图**、用户上传题图组、Vision 识题 → **IM1–IM5**，不单做 C3 |
| **约束** | 默认仍 warn + 换 Word；OCR 为 opt-in |
| **依赖候选** | Tesseract + `pytesseract`；Vision 需扩展 `llm_client` |
| **代码起点** | `document/extract_pdf.py`、`document/extract_images.py`（新建）、`document/image_read.py`（新建） |
| **验收** | 多页扫描 PDF fixture → 合并题干进 plan；无 OCR 依赖时 graceful 降级 |

**参考**：`V2_IMAGE_INPUT.md` §6–§7 · `LAB_SOLVER_AGENT_PLAN.md` PDF/OCR 章节
...
**参考**：`docs/KEY_STORAGE.md` §5（打包勿破坏 safeStorage IPC）
...
*V1 已有：`run_code`（py/js/c/cpp/java 单文件）、IDE/终端截图、UML 渲染。*
...
*日志脱敏已有 `log_util`；Key 存储见 `KEY_STORAGE.md`。*
...
| O30 | **识题预览（确认再 plan）** | 展示合并后的 `assignment_text`（含 OCR/多图），用户确认后 plan；粘贴/多文档解析后可在拆分预览区核对题目段 | **高** |
...
| O6 | **设置导出** | 须排除 Key（见 `KEY_STORAGE.md` §4.2） | 低 |
...
1. **DA** — 表格 / 节号 / 填表（[V2_DOC_TEMPLATE_ADAPTATION.md](./V2_DOC_TEMPLATE_ADAPTATION.md)）  
2. **IM** — 多图 OCR/Vision（[V2_IMAGE_INPUT.md](./V2_IMAGE_INPUT.md)；DA1 表格内图依赖 IM1）  
3. **O30 识题预览** — 低成本、显著减少「读错题还往下跑」；可与 IM2 同迭代  
4. **O12 Web 最小子集** — 若目标用户 Web 实训多，可与 L3 单独立项；否则文档标明「只生成代码」  
5. **打包 + CI**（§一 C3/PKG、CI）  
6. **C2 行为学习** — 靠后  
7. **§2.1–2.8 其余 O 项** — 按优先级与用户反馈插入，避免与 DA/IM 大改并行改 `fill_report`
...
```
在 lab-solver 只做 v2 Backlog 中的一项（见 docs/NEXT_VERSION_BACKLOG.md）：
...
| ID | 项 | 状态 | 备注 |
|----|-----|------|------|
| **DA** | **文档模版适配** | 📋 已立项 | [V2_DOC_TEMPLATE_ADAPTATION.md](./V2_DOC_TEMPLATE_ADAPTATION.md) |
| **IM** | **图片题目 + 多图处理** | 📋 已立项 | [V2_IMAGE_INPUT.md](./V2_IMAGE_INPUT.md) |
| IM1–IM5 | 枚举/OCR/PDF/上传/Vision | ⏳ 待做 | C3 并入 IM3 |
| **DS** | **动态分节工作台** | 📋 设计中 | [V2_DYNAMIC_SECTIONS.md](./V2_DYNAMIC_SECTIONS.md) — 从固定 4 节改为文档实际节结构 |
| C2 | 画像行为学习 | ⏳ v2 待做 | §一 |
| CI | GitHub Actions | ⏳ v2 待做 | §一 |
| PKG | 打包脚本 + `.exe` | ⏳ v2 待做 | §一 |
| O30 | 识题预览 | 📋 backlog | §2.7 **高** |
