...
**关联**: [LAB_SOLVER_AGENT_PLAN.md](LAB_SOLVER_AGENT_PLAN.md) · [AGENT_ERROR_HANDLING.md](AGENT_ERROR_HANDLING.md) · [AI_INSIGHTS.md](../reference/AI_INSIGHTS.md) · [V4_MULTI_PHASE_SOLVE.md](../product/V4_MULTI_PHASE_SOLVE.md) · [NEXT_VERSION_BACKLOG.md](../product/NEXT_VERSION_BACKLOG.md) §C2

> **V4 进展**：分阶段 `solve_lab`（`solve_pipeline.py`）已默认 v4；**AO-P0～P2（2026-06-06）** 已落地（深度去重、质量档位、run_mode 收敛为 standard/deep + 实验 ReAct、ReAct bootstrap V4、eval harness）。后续见 [AGENT_OPTIMIZATION_PLAN.md](AGENT_OPTIMIZATION_PLAN.md) AO-P3。
...
6. **进化层未闭环**：`../reference/AI_INSIGHTS.md` → 人工 promote → `skill_store`；backlog C2 行为学习未做。
...
| `ReactPolicy` | 多轮 LLM → parse action → `orchestrator.run_module` | 填表/交付逻辑本身 |
...
**ReAct 可调用工具**（`react_alias` 非空，2026-06-06 BF27 补全 `present_deliverable`）：

`solve_lab` · `run_code` · `fix_code` · `render_uml` · `fix_diagrams` · `present_deliverable` · `fill_report` · `finalize_report`

仅 Planner / 收尾可见、无 `react_alias` 的模块（如 `solve_theory`）不可在 ReAct 主循环内直接 ACTION 调用。
...
> **AO-7（2026-06-06）**：`run_react_loop` 进入 LLM 循环前 `_bootstrap_solve_lab_pipeline` 跑 V4 `solve_lab`；用户可见模式收敛见 AO-6（标准/深度 + 实验 ReAct）。
...
- [x] run_code
- [ ] render_uml
- [x] present_deliverable

规则：须先完成 solve_lab；默认以 present_deliverable 结束；若 run_code 多次失败，优先 finalize_report 而非无限 fix_code。
```
...
- 人工在 `../reference/AI_INSIGHTS.md` 标记 `<!-- skill-candidate -->`
...
**Promote 路径**：开发者确认 → 写入 `skill_store._register(...)` → 更新 `../reference/AI_INSIGHTS.md` §技能路径。
...
| `../product/NEXT_VERSION_BACKLOG.md` §C2 | V3-4 落地 C2 |
| `../reference/AI_INSIGHTS.md` | V3-4 skill 候选与 promote 流程 |
...
实施每个 V3 子阶段前，用 [PROMPT_CRITIQUE_CHECKLIST.md](../reference/PROMPT_CRITIQUE_CHECKLIST.md) 过一遍；额外加：
