...
**状态**: Phase 1-4 全部完成 ✅（2026-06-05）；**2026-06-06** 移除运行截图工具（V5-5）
...
后端已实现 8 个独立模块 + 3 个辅助工具，每个都有明确的输入/输出签名：
...
| 3 | ▶ 运行代码（高级） | `run_code` | 编译执行代码（Python/Java/C/Node） | 代码 + 语言 | stdout / stderr / exit_code |
| 4 | 📊 图表渲染 | `uml` + `dfd_render` | PlantUML / 标准 DFD → PNG（最多 12 张） | `diagrams` JSON 数组 / `plantuml_src` / `dfd_json` | PNG base64 + 类型统计 + 预览 |
| — | 📝 填写报告（实验性） | `fill_report` | 答案 JSON 写入 docx（**高级区**） | 答案 JSON + docx + fill_scope | 填写后的 docx |
| — | 🔧 修复代码 | `fix_code` | 编译错误自动修代码 | 代码 + 错误文本 + API Key | 修复后代码 + 解释 |
| — | ✅ 校验答案 | `quality.verify` | 规则校验 | 答案 JSON | 通过 / 问题列表 |
| — | ✏️ 修订答案 | `quality.revise` | LLM 局部/整题重写 | 答案 JSON + 反馈 + API Key | 修订后答案 |

> **2026-06-06 变更**：原 #4「IDE/终端运行截图」已移除（V5-5）。运行结果以 `result_description` 文字 + 可选内化验证 stdout 交付；填表插图仅支持 UML 与用户手动上传的 `images_b64`。
...
5. **辅助工具回写** — 「修复代码」成功后，修复结果自动写回 `#2` 解题 JSON 的 `code` / `code_files`，并同步到 `#3 运行代码` 输入框；`#3` / 填表 若已有结果则标记 stale，用户需重新执行 `#3` 验证修复是否有效（与 #4「AI 修复」图表后需重渲染同理）
6. **状态持久化** — 切换 Tab 不丢状态；切换文档后全部重置
7. **无依赖序列** — `#3` 与 `#4` 无先后依赖，可并行或单独执行
...
用户文档 (docx/pdf)
    ...
    └──[可选: 辅助工具 7/8/9 随时调用]──→ 修正中间结果
              ...
              │  🔧 修复代码 成功
              ▼
         写回 #2.code ──→ 同步 #3 输入 ──→ #3 / 填表 标记 stale
```
...
- **一键执行链** — 工具箱头部「⚡ 一键链 (#1→#2 解题)」：依次 `parse → solve`；填表在「高级 / 实验性」区手动执行，失败不阻断主链（V5-4）。
...
#### 4.1 后端测试覆盖（`tests/test_toolbox.py`）

10 个测试类覆盖全部工具路由 + 辅助函数（**2026-06-06** 起不再含 `TestToolScreenshot`）：

- `TestToolHelpers` — `_tool_ok` / `_tool_err` / `_tool_settings`
- `TestToolParse` — 解析 doc/docx/pdf
- `TestToolSolve` — AI 解题
- `TestToolRun` — 运行代码
- `TestToolUml` — PlantUML + DFD 图表渲染
...
| 辅助工具 | 回写行为 |
|----------|----------|
| 🔧 修复代码 | `propagateFixedCodeToToolbox()` — 合并 `code` / `code_files` / `main_file` / `language` 到 `toolState.solve.output`；更新 `toolState.run.input`；`#3` / 填表 在 success 或 failed 时改为 stale |
| 🛠 AI 修复（#4） | `fixDiagramsTool()` — 合并 `answer_json` 到 solve；更新 uml 输入；`markDownstreamStale('solve')` |
...
| `propagateFixedCodeToToolbox()` | 修复代码成功后写回 solve + run |
| `showReviseFeedbackModal()` | Promise-based 修订反馈模态框 |
| `confirmResetToolbox()` | 带确认的重置入口 |

### Phase 5 ✅ — 图表扩展（Phase C 对齐，2026-06-05）

与 [DIAGRAM_EXPANSION_PLAN.md](../features/DIAGRAM_EXPANSION_PLAN.md) Phase C 同步，工具箱 #5 从「单张 PlantUML」升级为 **图表渲染**：
...
*文档版本：2026-06-06（V5-4：#6 填表移入高级区；主链 #1→#2）*
