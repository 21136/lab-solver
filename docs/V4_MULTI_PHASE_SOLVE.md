...
**关联**: [V5_PRODUCT_PIVOT.md](./V5_PRODUCT_PIVOT.md) · [AI_INSIGHTS.md](./AI_INSIGHTS.md) · [AGENT_ERROR_HANDLING.md](./AGENT_ERROR_HANDLING.md) · [V2_TOOLBOX_MODE.md](./V2_TOOLBOX_MODE.md) · [AGENT_ARCHITECTURE_V3.md](./AGENT_ARCHITECTURE_V3.md) · [NEXT_VERSION_BACKLOG.md](./NEXT_VERSION_BACKLOG.md)
...
3. **补救型多轮**：`deep_pipeline` 的 preflight/fix、`executor` 的 `_fix_and_retry` 都是**坏了再修**，不是**分阶段生成**。
...
| ID     | 目标                                                         | 验收                                  |
| ------ | ---------------------------------------------------------- | ----------------------------------- |
| **G1** | 代码首次运行通过率（简单算法类实验）从 ~40% 提升到 **≥80%**                      | 金样本 10 份 docx fixture，mock 外真跑      |
...
- ReAct 模式 V4 第一版可降级为「Pipeline + 收尾工具」，完整 ReAct 多轮推理放到 V4.2
...
| **深度 Agent** | 删除 `deep_pipeline` 内重复的 preflight/fix 环；保留 **reflect → revise** 但作用于 Phase 2 文字（不碰已验证代码） |
| **ReAct**    | V4.1：首屏直接 `SolvePipeline`，成功后 `finalize_report`；V4.2 再考虑把 phase 拆成 tool                  |
...
src/python/
  ...
  agent/
    ...
    deep_pipeline.py         # 删除重复 fix 环
  tests/
    test_solve_pipeline.py # 金样本 + mock LLM（NEW）
    fixtures/solve_v4/     # 10 份简单实验 docx + 期望 code_status
...
- SSE `pipeline_phase` + Step3 子进度条  
- `deep_pipeline` 删重复环；reflect 只修订文字  
- Planner 默认计划隐藏独立 `run_code`  
- 金样本回归 10 份
...
| -------------- | ----------- | --------------- |
| 首次 run 通过率     | ~40%        | ≥80%            |
| 平均 LLM 次数（简单题） | 1 + 2.5 fix | 3～4（含报告）        |
...
| **前端** | 设置页「Java 扩展库」列表；下载确认模态（名称、大小、课程用途）；与 JRE 引导同一风格 |
