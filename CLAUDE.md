...
桌面 Electron + Python Flask 后端实验报告解题助手。用户自填 LLM API Key（支持 DeepSeek/OpenAI/Claude/自定义），上传实验报告 doc/docx/pdf（旧版 .doc 自动转换），AI 自动解题、运行代码、截图、填回 Word。
...
| Key 存储 | Electron safeStorage 加密 → localStorage，详见 `docs/KEY_STORAGE.md` |
...
│       │   ├── deep_pipeline.py     # DeepPipeline：understand+plan→draft→preflight→reflect→revise
...
│       │   ├── react_finalize.py     # ReAct 收尾：自动 UML/截图/填表 + finalize_report 工具
│       │   ├── react_tools.py        # ReAct 工具：solve/fix/run/screenshot/uml/fill/finalize_report
...
│       │   ├── user_profile.py      # 用户画像 v1（default_language/screenshot_style/prefer_uml）
...
│       │   ├── solve_lab.py         # LLM 生成结构化实验答案
│       │   ├── run_code.py          # 编译运行代码
│       │   ├── screenshot.py        # IDE/终端截图
│       │   ├── uml.py               # 图表提取与渲染（PlantUML + DFD，最多 12 张）
│       │   ├── fill_report.py       # 写回 docx；training_table 分格填表 + 单元格内嵌 UML/截图
...
│   ├── generate_fixtures.py        # 生成 4 份金样本 docx
│   ├── run_golden_regression.py    # 金样本回归脚本
│   ├── fixtures/                   # programming_lab / theory_lab / combined_lab / training_table
...
│   ├── test_log_util.py
│   ├── test_image_input.py          # IM1 图片提取测试（34 个）
...
| 文档 | 用途 |
|------|------|
| `LAB_SOLVER_AGENT_PLAN.md` | **完整架构设计**（主文档）：Agent、DeepPipeline、**ReAct+收尾**、分节工作台、API |
| `AGENT_ARCHITECTURE_V3.md` | **Agent 架构加强（V3）**：V3-1 ✅ registry + ReAct LLM/读题；待做 Orchestrator、auto_remediate、C2 |
| `IMPLEMENTATION_PHASES.md` | V1 分阶段实施拆分 + Phase V3 进度（§五，V3-1 ✅） |
| `PLAN_EXPORT_README.md` | 计划要点速览 + 给 AI 的提问模板 |
| `NEXT_VERSION_BACKLOG.md` | v2 backlog：DA/IM 立项 + O7-O32 盲区 + 发布基建 |
| `V2_DOC_TEMPLATE_ADAPTATION.md` | v2 DA1-DA4：表格实训、节号语义映射、填表适配 |
| `V2_IMAGE_INPUT.md` | v2 IM1-IM5：多图枚举/OCR/Vision |
| `ENVIRONMENT_PROBE.md` | L4 运行时环境探测设计：Python/Java/C/Node 探测 → prompt 注入 |
| `AGENT_ERROR_HANDLING.md` | Agent 错误处理增强：预检→分类→策略修复→智能降级 |
| `deepseek的建议.md` | DeepSeek 13 条评审建议（已合并主计划） |
| `PROMPT_CRITIQUE_CHECKLIST.md` | 7 维度评审清单 |
| `AI_INSIGHTS.md` | LLM 解题自述洞察收集 + 技能学习路径（skill_store.py 注册 3 技能，executor 自动追加 Agent 模式 notes） |
| `KEY_STORAGE.md` | safeStorage 加密方案 + 风险说明 |
| `V2_TOOLBOX_MODE.md` | 工具箱模式设计文档：独立工具 API + 前端面板（含 #5 图表渲染 / DFD） |
| `V5_PRODUCT_PIVOT.md` | **V5 战略大改**：生成优先、验证内化、用户落笔；Deliverable 主输出 |
| `V4_MULTI_PHASE_SOLVE.md` | V4 分阶段 LLM 流水线（技术子方案，并入 V5-1） |
| `DIAGRAM_EXPANSION_PLAN.md` | 图表扩展：UML 种类、DFD、便携 Graphviz、Agent vs 工具箱分工 |
| `V1_BUGFIX_LOG.md` | 运行时 bug 修复记录（BF1–BF22，含填表/ReAct 收尾/思考过程导出） |
...
**运行模式**：`run_mode=standard` (~2 次 LLM)；`run_mode=deep` (DeepPipeline ~3-4 次)；`run_mode=react` (ReAct Loop ~5-12 次，**循环结束后自动补跑** UML/截图/填表；工具 `finalize_report` 一键收尾)。

**工具箱模式**：Step 2 顶部模式切换 Tab（引导模式 / 工具箱模式）。工具箱提供 9 个独立工具卡片，用户按推荐顺序手动执行：解析文档 → AI 解题 → 运行代码 / 截图 / **图表渲染** → 填写报告，以及 3 个辅助工具（修复代码、校验答案、修订答案）。#5 **图表渲染**支持 `diagrams` JSON 数组（PlantUML + 标准 DFD，最多 12 张），头部展示 PlantUML/Java/Graphviz 引擎状态。上游工具输出自动传递为下游默认输入；**修复代码**成功后会写回 `#2` 并同步到 `#3 运行代码` 输入框（`propagateFixedCodeToToolbox`）。详见 `docs/V2_TOOLBOX_MODE.md`。

**V5 产品大改（V5-0 已落地）**：定位从「代跑+代填 Word」转为 **生成答案内容（Deliverable）由用户自行落笔**；默认 `output_mode=deliverable` + Step3 答案工作区；`fill_report` 降为实验性高级功能。详见 `docs/V5_PRODUCT_PIVOT.md`。

**V4 分阶段解题（技术子方案）**：读题 → 只写代码 → 内化验证 → 写报告 → 图表；并入 V5-1。详见 `docs/V4_MULTI_PHASE_SOLVE.md`。

**工具箱 API** (9 条路由，统一 `{ok, data, error}` 响应)：

| 路由 | 功能 | 模块 |
|------|------|------|
| `POST /api/tool/parse` | 解析 doc/docx/pdf（.doc 自动转换） | `parse_report` |
| `POST /api/tool/solve` | AI 解题 | `solve_lab` |
| `POST /api/tool/run` | 运行代码 | `run_code` |
| `POST /api/tool/screenshot` | IDE 截图 | `screenshot` |
...
- **react-finalize** ✅ — ReAct 自动收尾（UML/截图/填表）+ `finalize_report` 一键工具
- IM2-IM5 ⏸ — 批量 OCR/Vision/UI 上传，等 DeepSeek 支持 Vision 后恢复
